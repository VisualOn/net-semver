...
gulp.task 'pack', ['build'], ->
  ...
  gulp.src nuspecPath, read: false
    .pipe shell ["mkdir build"]
    .pipe shell ["nuget pack <%= f(file.path) %> -o build -version #{version} -p Configuration=#{configuration}"],
      templateData:
        f: (s) -> s.replace '\\', '/'

gulp.task 'push', ->
  version = getCurrentVersion()
  gulp.src "./build/SemanticVersioning-#{version}.nupkg"
